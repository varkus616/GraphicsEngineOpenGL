    ...
    currentRenderData.uniformUpdater = [&](Renderable& r, RenderData& d, Window& w) -> void
        {
            ...
            d.shaderConfig.uniforms["spotLight.position"] = UniformValue(UniformType::VEC3, viewPos);
            d.shaderConfig.uniforms["spotLight.direction"] = UniformValue(UniformType::VEC3, camDir);
            d.shaderConfig.uniforms["spotLight.ambient"] = UniformValue(UniformType::VEC3, light.ambient);
            d.shaderConfig.uniforms["spotLight.diffuse"] = UniformValue(UniformType::VEC3, light.diffuse);
            d.shaderConfig.uniforms["spotLight.specular"] = UniformValue(UniformType::VEC3, light.specular);
            
            d.shaderConfig.uniforms["spotLight.constant"] = UniformValue(UniformType::FLOAT, light.constant);
            d.shaderConfig.uniforms["spotLight.linear"] = UniformValue(UniformType::FLOAT, light.linear);
            d.shaderConfig.uniforms["spotLight.quadratic"] = UniformValue(UniformType::FLOAT, light.quadratic);
            d.shaderConfig.uniforms["spotLight.cutOff"] = UniformValue(UniformType::FLOAT, light.cutOff);
...
void App::update()
{
    ...
    pointLights[0].position = m_window.getCamera().GetPosition();
...
void App::renderImGui()
{
    ...
    ImGui::ColorEdit3("Light Ambient", &pointLights[0].ambient[0]);
    ImGui::ColorEdit3("Light Diffuse", &pointLights[0].diffuse[0]);
    ImGui::ColorEdit3("Light Specular", &pointLights[0].specular[0]);
